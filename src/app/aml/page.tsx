...
export default function AMLPage() {
  ...
  async function handleDownloadCSV() {
    if (selectedTransactions.size === 0) {
      setMessage('Please select at least one transaction to download');
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `goAML-Export-${new Date().toISOString().slice(0, 10)}.csv`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
        setMessage(`Downloaded CSV for ${selectedTransactions.size} transaction(s)`);
        setTimeout(() => setMessage(''), 3000);
      } else {
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              {activeTab === 'goaml' && transactions.length > 0 && (
                <div className="flex gap-2">
                  ...
                  >
                    Download CSV ({selectedTransactions.size})
                  </button>
                  <button
